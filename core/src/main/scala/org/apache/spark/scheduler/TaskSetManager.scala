...
import org.apache.spark.scheduler.TaskPreemptionUtil.SPARK_PREEMPT_THRESHOLD
...
private[spark] class TaskSetManager(
  ...
  val preemptThreshold = Option(conf.getInt(SPARK_PREEMPT_THRESHOLD, 0)).filter(_ > 0)
