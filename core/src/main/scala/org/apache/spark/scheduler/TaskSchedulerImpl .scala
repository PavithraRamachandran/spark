...
private[spark] class TaskSchedulerImpl(
 ...
 val canPreempt =  checkPreemptionFlag && SchedulingMode.FAIR.equals(this.schedulingMode)
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    breakable {
      for (taskSet <- sortedTaskSets) {
        ...
        logError("")
        if (taskSet.sparkExecutionId.isDefined) {
    ...
    val accumUpdatesWithTaskIds: Array[(Long, Int, Int, Seq[AccumulableInfo])] = {
      accumUpdates.flatMap {
        case (id, updates) =>
          ...
          Option(taskIdToTaskSetManager.get(id)).map {
            taskSetMgr =>
  ...
  private def removeExecutor(executorId: String, reason: ExecutorLossReason) {
    ...
    executorIdToRunningTaskIds.remove(executorId).foreach {
      taskIds =>
        logDebug("Cleaning up TaskScheduler state for tasks " +
          s"${
            taskIds.mkString("[", ",", "]")
          } on failed executor $executorId ")
